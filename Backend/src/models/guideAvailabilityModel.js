...
export async function getAllAvailabilities() {
    const select = 'SELECT * FROM guide_availability ORDER BY available_date ASC';
    const [result] = await connection.query(select);
    return result;
};

export async function getAvailabilityByGuide(guide_id) {
    const select = 'SELECT * FROM guide_availability WHERE guide_id = ? ORDER BY available_date ASC';
    const [result] = await connection.query(select, [guide_id]);
    return result;
};

export async function getAvailabilityById(id) {
    const select = 'SELECT * FROM guide_availability WHERE available_id = ?';
